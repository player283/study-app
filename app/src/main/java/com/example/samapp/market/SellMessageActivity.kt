...
import com.example.samapp.model.ChatModel
import com.example.samapp.model.User
import com.google.firebase.database.ktx.getValue

class SellMessageActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_sell_message)
        ...
        imageView.setOnClickListener {
            ...
            if(chatRoomUid == null){
                ...
                fireDatabase.child("chatrooms").push().setValue(chatModel).addOnSuccessListener {
                    ...
                    //메세지 보내기
                    Handler().postDelayed({
                        val messageActivityText = findViewById<TextView>(R.id.messageActivity_editText)
                        println(chatRoomUid)
                        fireDatabase.child("chatrooms").child(chatRoomUid.toString()).child("comments").push().setValue(comment)
                        messageActivityText.text=null
                    }, 1000L)
                    Log.d("chatUidNull dest", "$destinationUid,$uid")
                }
            }else{
                val messageActivityText = findViewById<TextView>(R.id.messageActivity_editText)
                fireDatabase.child("chatrooms").child(chatRoomUid.toString()).child("comments").push().setValue(comment)
    ...
    private fun checkChatRoom(){
        val messageActivityImageView = findViewById<ImageView>(R.id.messageActivity_ImageView)
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    for (item in snapshot.children){
                        ...
                        if(chatModel?.users!!.containsKey(destinationUid)){
                            ...
                            messageActivityImageView.isEnabled = true
                            recyclerView = findViewById(R.id.messageActivity_recyclerview)
                            recyclerView?.layoutManager = LinearLayoutManager(this@SellMessageActivity)
    ...
    inner class RecyclerViewAdapter : RecyclerView.Adapter<RecyclerViewAdapter.MessageViewHolder>() {
        val chatingName = findViewById<TextView>(R.id.chatingName)
